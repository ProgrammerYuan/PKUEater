...
public class HttpHandler<T> extends PriorityAsyncTask<Object, Object, Void> implements RequestCallBackHandler {
    ...
    public HttpHandler(AbstractHttpClient client, HttpContext context, String charset, RequestCallBack<T> callback, String url, LinkedHashMap<String, String> map) {
        ...
        if (param != null) {
            for (Map.Entry en : param.entrySet()) {
                try {
                    para.put(en.getKey().toString(), en.getValue().toString());
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    protected void onProgressUpdate(Object... values) {
        ...
        switch ((Integer) values[0]) {
            ...
            case UPDATE_FAILURE:
                ...
                callback.onFailure((HttpException) values[1], (String) values[2],data);
                break;
            case UPDATE_SUCCESS:
                if (values.length != 2) return;
                this.state = State.SUCCESS;
                callback.onSuccess((ResponseInfo<T>) values[1],data);
                break;
            default:
                break;
        }
