 ...
 */
public class FlipperIndicator extends LinearLayout {
    ArrayList<FlipperIndicatorUnit> unitList;
    //int drawable_selected, drawable_unselected;

    /**
     * @param context
     */
    public FlipperIndicator(Context context) {
        super(context);
        init(context);
    }
     ...
     * @param attrs
     */
    public FlipperIndicator(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    public void init(Context c) {
        ...
        unitList = new ArrayList<FlipperIndicatorUnit>();
    }
    ...
    public void setUnitCount(int count, int drawable_selected, int drawable_unselected) {
        ...
        for (int i = 0; i < count; i++) {
            boolean b = (i == 0);
            FlipperIndicatorUnit fiu = new FlipperIndicatorUnit(getContext(), b, drawable_selected, drawable_unselected);
            unitList.add(fiu);
            this.addView(fiu);
        }
    }
    ...
    public void setSelectedUnit(int index) {
        ...
        for (int i = 0; i < unitList.size(); i++) {
            try {
                ...
                FlipperIndicatorUnit flu = unitList.get(i);
                if (i == index) {
                    flu.setStat(true);
                } else {
                    flu.setStat(false);
...
//	
//	public void setStyle(int selected,int unselected){
//		drawable_selected=selected;
//		drawable_unselected=unselected;
//		for (int i = 0; i < unitList.size(); i++) {
//			try {
//
//				FlipperIndicatorUnit flu = unitList.get(i);
//				flu.drawable_selected=drawable_selected;
//				flu.drawable_unselected=drawable_unselected;
//				
//			} catch (Exception e) {
//				e.printStackTrace();
//			}
...
class FlipperIndicatorUnit extends RelativeLayout {
     ...
     * @param context
     */
    public FlipperIndicatorUnit(Context context, boolean def, int c_s, int c_us) {
        super(context);
     ...
     * @param attrs
     */
    public FlipperIndicatorUnit(Context context, AttributeSet attrs, boolean def) {
        super(context, attrs);
        init(context, def);
    }
