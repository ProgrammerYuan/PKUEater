...
public class JazzyViewPager extends EdgeEffectViewPager {
    ...
    boolean indicator_supressed = false;
    int indicator_current = 0;
...
//		reset();
    ...
    private View mLeft;
    private View mRight;
    private PagerTabIndicator mLeftIndicator;
    private PagerTabIndicator mRightIndicator;
    private ArrayList<PagerTabIndicator> indicators;
    private float mRot;
...
//}
    ...
    //    private void animateFade(View left, View right, float positionOffset, boolean in) {
//        if (mState != State.IDLE) {
//            if (left != null) {
//                manageLayer(left, true);
//                mAlpha = in ? FADE_MIN + (1 - FADE_MIN) * (1 - positionOffset) :
//                        (1 - FADE_MIN )* (1 - positionOffset);
//                left.setPivotX(left.getMeasuredWidth() * 0.5f);
//                left.setPivotY(left.getMeasuredHeight() * 0.5f);
//                left.setScaleX(mAlpha);
//                left.setScaleY(mAlpha);
//            }
//            if (right != null) {
//                manageLayer(right, true);
//                mAlpha = in ? ZOOM_MAX + (1 - ZOOM_MAX) * positionOffset :
//                        1 + ZOOM_MAX - ZOOM_MAX * positionOffset;
//                right.setPivotX(right.getMeasuredWidth() * 0.5f);
//                right.setPivotY(right.getMeasuredHeight() * 0.5f);
//                right.setScaleX(mAlpha);
//                right.setScaleY(mAlpha);
//            }
//        }
//    }
    ...
    public void setIndicators(ArrayList<PagerTabIndicator> l) {
        indicators = l;
    }

    @Override
    public void setCurrentItem(int item) {
        indicator_supressed = true;
        indicator_current=getCurrentItem();
        super.setCurrentItem(item);
//        indicator_supressed=false;
    }
...
//		mRight = getChildAt(position+1);
        ...
        if (indicators != null && !indicator_supressed) {
            try {
                mLeftIndicator = indicators.get(position);
                mRightIndicator = indicators.get(position + 1);
                if (mLeftIndicator != null) {
                    mLeftIndicator.onScroll(positionOffset);
                }
                if (mRightIndicator != null) {
                    mRightIndicator.onScroll(1 - positionOffset);
                }
            } catch (Exception e) {
//                e.printStackTrace();
            }
...
//            }
        ...
        if (effectOffset == 0) {
            ...
            mState = State.IDLE;
            if (indicator_supressed) {
                if (indicators != null) {
                    try {
                        mLeftIndicator = indicators.get(position);
                        mRightIndicator = indicators.get(indicator_current);
                        if (mLeftIndicator != null) {
                            mLeftIndicator.onScroll(0);
                        }
                        if (mRightIndicator != null) {
                            mRightIndicator.onScroll(1);
                        }
                    } catch (Exception e) {
//                e.printStackTrace();
                    }
